...
!(function(global) {                                                                                               // 3525
  ...
  var runtime = global.regeneratorRuntime;                                                                         // 3534
                                                                                                                   // 3535
  // pb 2016-03-29: Allow re-defining of default runtime provided with                                             // 3536
  //                Meteor 1.3, because it does not include some cruicial                                          // 3537
  //                methods, such as awrap!                                                                        // 3538
  if (false && runtime) {                                                                                          // 3539
    if (inModule) {                                                                                                // 3540
      // If regeneratorRuntime is defined globally and we're in a module,                                          // 3541
      // make the exports object identical to regeneratorRuntime.                                                  // 3542
      module.exports = runtime;                                                                                    // 3543
    }                                                                                                              // 3544
    // Don't bother evaluating the rest of this file if the runtime was                                            // 3545
    // already defined globally.                                                                                   // 3546
    return;                                                                                                        // 3547
  }                                                                                                                // 3548
                                                                                                                   // 3549
  // Define the runtime globally (as expected by generated code) as either                                         // 3550
  // module.exports (if we're in a module) or a new, empty object.                                                 // 3551
  runtime = global.regeneratorRuntime = inModule ? module.exports : {};                                            // 3552
                                                                                                                   // 3553
  function wrap(innerFn, outerFn, self, tryLocsList) {                                                             // 3554
    // If outerFn provided, then outerFn.prototype instanceof Generator.                                           // 3555
    var generator = Object.create((outerFn || Generator).prototype);                                               // 3556
    var context = new Context(tryLocsList || []);                                                                  // 3557
                                                                                                                   // 3558
    // The ._invoke method unifies the implementations of the .next,                                               // 3559
    // .throw, and .return methods.                                                                                // 3560
    generator._invoke = makeInvokeMethod(innerFn, self, context);                                                  // 3561
                                                                                                                   // 3562
    return generator;                                                                                              // 3563
  }                                                                                                                // 3564
  runtime.wrap = wrap;                                                                                             // 3565
                                                                                                                   // 3566
  // Try/catch helper to minimize deoptimizations. Returns a completion                                            // 3567
  // record like context.tryEntries[i].completion. This interface could                                            // 3568
  // have been (and was previously) designed to take a closure to be                                               // 3569
  // invoked without arguments, but in all the cases we care about we                                              // 3570
  // already have an existing method we want to call, so there's no need                                           // 3571
  // to create a new function object. We can even get away with assuming                                           // 3572
  // the method takes exactly one argument, since that happens to be true                                          // 3573
  // in every case, so we don't have to touch the arguments object. The                                            // 3574
  // only additional allocation required is the completion record, which                                           // 3575
  // has a stable shape and so hopefully should be cheap to allocate.                                              // 3576
  function tryCatch(fn, obj, arg) {                                                                                // 3577
    try {                                                                                                          // 3578
      return { type: "normal", arg: fn.call(obj, arg) };                                                           // 3579
    } catch (err) {                                                                                                // 3580
      return { type: "throw", arg: err };                                                                          // 3581
    }                                                                                                              // 3582
  }                                                                                                                // 3583
                                                                                                                   // 3584
  var GenStateSuspendedStart = "suspendedStart";                                                                   // 3585
  var GenStateSuspendedYield = "suspendedYield";                                                                   // 3586
  var GenStateExecuting = "executing";                                                                             // 3587
  var GenStateCompleted = "completed";                                                                             // 3588
                                                                                                                   // 3589
  // Returning this object from the innerFn has the same effect as                                                 // 3590
  // breaking out of the dispatch switch statement.                                                                // 3591
  var ContinueSentinel = {};                                                                                       // 3592
                                                                                                                   // 3593
  // Dummy constructor functions that we use as the .constructor and                                               // 3594
  // .constructor.prototype properties for functions that return Generator                                         // 3595
  // objects. For full spec compliance, you may wish to configure your                                             // 3596
  // minifier not to mangle the names of these two functions.                                                      // 3597
  function Generator() {}                                                                                          // 3598
  function GeneratorFunction() {}                                                                                  // 3599
  function GeneratorFunctionPrototype() {}                                                                         // 3600
                                                                                                                   // 3601
  var Gp = GeneratorFunctionPrototype.prototype = Generator.prototype;                                             // 3602
  GeneratorFunction.prototype = Gp.constructor = GeneratorFunctionPrototype;                                       // 3603
  GeneratorFunctionPrototype.constructor = GeneratorFunction;                                                      // 3604
  GeneratorFunction.displayName = "GeneratorFunction";                                                             // 3605
                                                                                                                   // 3606
  // Helper for defining the .next, .throw, and .return methods of the                                             // 3607
  // Iterator interface in terms of a single ._invoke method.                                                      // 3608
  function defineIteratorMethods(prototype) {                                                                      // 3609
    ["next", "throw", "return"].forEach(function(method) {                                                         // 3610
      prototype[method] = function(arg) {                                                                          // 3611
        return this._invoke(method, arg);                                                                          // 3612
      };                                                                                                           // 3613
    });                                                                                                            // 3614
  }                                                                                                                // 3615
                                                                                                                   // 3616
  runtime.isGeneratorFunction = function(genFun) {                                                                 // 3617
    var ctor = typeof genFun === "function" && genFun.constructor;                                                 // 3618
    return ctor                                                                                                    // 3619
      ? ctor === GeneratorFunction ||                                                                              // 3620
        // For the native GeneratorFunction constructor, the best we can                                           // 3621
        // do is to check its .name property.                                                                      // 3622
        (ctor.displayName || ctor.name) === "GeneratorFunction"                                                    // 3623
      : false;                                                                                                     // 3624
  };                                                                                                               // 3625
                                                                                                                   // 3626
  runtime.mark = function(genFun) {                                                                                // 3627
    if (Object.setPrototypeOf) {                                                                                   // 3628
      Object.setPrototypeOf(genFun, GeneratorFunctionPrototype);                                                   // 3629
    } else {                                                                                                       // 3630
      genFun.__proto__ = GeneratorFunctionPrototype;                                                               // 3631
    }                                                                                                              // 3632
    genFun.prototype = Object.create(Gp);                                                                          // 3633
    return genFun;                                                                                                 // 3634
  };                                                                                                               // 3635
                                                                                                                   // 3636
  // Within the body of any async function, `await x` is transformed to                                            // 3637
  // `yield regeneratorRuntime.awrap(x)`, so that the runtime can test                                             // 3638
  // `value instanceof AwaitArgument` to determine if the yielded value is                                         // 3639
  // meant to be awaited. Some may consider the name of this method too                                            // 3640
  // cutesy, but they are curmudgeons.                                                                             // 3641
  runtime.awrap = function(arg) {                                                                                  // 3642
    return new AwaitArgument(arg);                                                                                 // 3643
  };                                                                                                               // 3644
                                                                                                                   // 3645
  function AwaitArgument(arg) {                                                                                    // 3646
    this.arg = arg;                                                                                                // 3647
  }                                                                                                                // 3648
                                                                                                                   // 3649
  function AsyncIterator(generator) {                                                                              // 3650
    // This invoke function is written in a style that assumes some                                                // 3651
    // calling function (or Promise) will handle exceptions.                                                       // 3652
    function invoke(method, arg) {                                                                                 // 3653
      var result = generator[method](arg);                                                                         // 3654
      var value = result.value;                                                                                    // 3655
      return value instanceof AwaitArgument                                                                        // 3656
        ? Promise.resolve(value.arg).then(invokeNext, invokeThrow)                                                 // 3657
        : Promise.resolve(value).then(function(unwrapped) {                                                        // 3658
            // When a yielded Promise is resolved, its final value becomes                                         // 3659
            // the .value of the Promise<{value,done}> result for the                                              // 3660
            // current iteration. If the Promise is rejected, however, the                                         // 3661
            // result for this iteration will be rejected with the same                                            // 3662
            // reason. Note that rejections of yielded Promises are not                                            // 3663
            // thrown back into the generator function, as is the case                                             // 3664
            // when an awaited Promise is rejected. This difference in                                             // 3665
            // behavior between yield and await is important, because it                                           // 3666
            // allows the consumer to decide what to do with the yielded                                           // 3667
            // rejection (swallow it and continue, manually .throw it back                                         // 3668
            // into the generator, abandon iteration, whatever). With                                              // 3669
            // await, by contrast, there is no opportunity to examine the                                          // 3670
            // rejection reason outside the generator function, so the                                             // 3671
            // only option is to throw it from the await expression, and                                           // 3672
            // let the generator function handle the exception.                                                    // 3673
            result.value = unwrapped;                                                                              // 3674
            return result;                                                                                         // 3675
          });                                                                                                      // 3676
    }                                                                                                              // 3677
                                                                                                                   // 3678
    if (typeof process === "object" && process.domain) {                                                           // 3679
      invoke = process.domain.bind(invoke);                                                                        // 3680
    }                                                                                                              // 3681
                                                                                                                   // 3682
    var invokeNext = invoke.bind(generator, "next");                                                               // 3683
    var invokeThrow = invoke.bind(generator, "throw");                                                             // 3684
    var invokeReturn = invoke.bind(generator, "return");                                                           // 3685
    var previousPromise;                                                                                           // 3686
                                                                                                                   // 3687
    function enqueue(method, arg) {                                                                                // 3688
      function callInvokeWithMethodAndArg() {                                                                      // 3689
        return invoke(method, arg);                                                                                // 3690
      }                                                                                                            // 3691
                                                                                                                   // 3692
      return previousPromise =                                                                                     // 3693
        // If enqueue has been called before, then we want to wait until                                           // 3694
        // all previous Promises have been resolved before calling invoke,                                         // 3695
        // so that results are always delivered in the correct order. If                                           // 3696
        // enqueue has not been called before, then it is important to                                             // 3697
        // call invoke immediately, without waiting on a callback to fire,                                         // 3698
        // so that the async generator function has the opportunity to do                                          // 3699
        // any necessary setup in a predictable way. This predictability                                           // 3700
        // is why the Promise constructor synchronously invokes its                                                // 3701
        // executor callback, and why async functions synchronously                                                // 3702
        // execute code before the first await. Since we implement simple                                          // 3703
        // async functions in terms of async generators, it is especially                                          // 3704
        // important to get this right, even though it requires care.                                              // 3705
        previousPromise ? previousPromise.then(                                                                    // 3706
          callInvokeWithMethodAndArg,                                                                              // 3707
          // Avoid propagating failures to Promises returned by later                                              // 3708
          // invocations of the iterator.                                                                          // 3709
          callInvokeWithMethodAndArg                                                                               // 3710
        ) : new Promise(function (resolve) {                                                                       // 3711
          resolve(callInvokeWithMethodAndArg());                                                                   // 3712
        });                                                                                                        // 3713
    }                                                                                                              // 3714
                                                                                                                   // 3715
    // Define the unified helper method that is used to implement .next,                                           // 3716
    // .throw, and .return (see defineIteratorMethods).                                                            // 3717
    this._invoke = enqueue;                                                                                        // 3718
  }                                                                                                                // 3719
                                                                                                                   // 3720
  defineIteratorMethods(AsyncIterator.prototype);                                                                  // 3721
                                                                                                                   // 3722
  // Note that simple async functions are implemented on top of                                                    // 3723
  // AsyncIterator objects; they just return a Promise for the value of                                            // 3724
  // the final result produced by the iterator.                                                                    // 3725
  runtime.async = function(innerFn, outerFn, self, tryLocsList) {                                                  // 3726
    var iter = new AsyncIterator(                                                                                  // 3727
      wrap(innerFn, outerFn, self, tryLocsList)                                                                    // 3728
    );                                                                                                             // 3729
                                                                                                                   // 3730
    return runtime.isGeneratorFunction(outerFn)                                                                    // 3731
      ? iter // If outerFn is a generator, return the full iterator.                                               // 3732
      : iter.next().then(function(result) {                                                                        // 3733
          return result.done ? result.value : iter.next();                                                         // 3734
        });                                                                                                        // 3735
  };                                                                                                               // 3736
                                                                                                                   // 3737
  function makeInvokeMethod(innerFn, self, context) {                                                              // 3738
    var state = GenStateSuspendedStart;                                                                            // 3739
                                                                                                                   // 3740
    return function invoke(method, arg) {                                                                          // 3741
      if (state === GenStateExecuting) {                                                                           // 3742
        throw new Error("Generator is already running");                                                           // 3743
      }                                                                                                            // 3744
                                                                                                                   // 3745
      if (state === GenStateCompleted) {                                                                           // 3746
        if (method === "throw") {                                                                                  // 3747
          throw arg;                                                                                               // 3748
        }                                                                                                          // 3749
                                                                                                                   // 3750
        // Be forgiving, per 25.3.3.3.3 of the spec:                                                               // 3751
        // https://people.mozilla.org/~jorendorff/es6-draft.html#sec-generatorresume                               // 3752
        return doneResult();                                                                                       // 3753
      }                                                                                                            // 3754
                                                                                                                   // 3755
      while (true) {                                                                                               // 3756
        var delegate = context.delegate;                                                                           // 3757
        if (delegate) {                                                                                            // 3758
          if (method === "return" ||                                                                               // 3759
              (method === "throw" && delegate.iterator[method] === undefined)) {                                   // 3760
            // A return or throw (when the delegate iterator has no throw                                          // 3761
            // method) always terminates the yield* loop.                                                          // 3762
            context.delegate = null;                                                                               // 3763
                                                                                                                   // 3764
            // If the delegate iterator has a return method, give it a                                             // 3765
            // chance to clean up.                                                                                 // 3766
            var returnMethod = delegate.iterator["return"];                                                        // 3767
            if (returnMethod) {                                                                                    // 3768
              var record = tryCatch(returnMethod, delegate.iterator, arg);                                         // 3769
              if (record.type === "throw") {                                                                       // 3770
                // If the return method threw an exception, let that                                               // 3771
                // exception prevail over the original return or throw.                                            // 3772
                method = "throw";                                                                                  // 3773
                arg = record.arg;                                                                                  // 3774
                continue;                                                                                          // 3775
              }                                                                                                    // 3776
                                                                                                                   ...
                                                                                                                   // 3778
            if (method === "return") {                                                                             // 3779
              // Continue with the outer return, now that the delegate                                             // 3780
              // iterator has been terminated.                                                                     // 3781
              continue;                                                                                            // 3782
            }                                                                                                      // 3783
          }                                                                                                        // 3784
                                                                                                                   // 3785
          var record = tryCatch(                                                                                   // 3786
            delegate.iterator[method],                                                                             // 3787
            delegate.iterator,                                                                                     // 3788
            arg                                                                                                    // 3789
          );                                                                                                       // 3790
                                                                                                                   // 3791
          if (record.type === "throw") {                                                                           // 3792
            context.delegate = null;                                                                               // 3793
                                                                                                                   // 3794
            // Like returning generator.throw(uncaught), but without the                                           // 3795
            // overhead of an extra function call.                                                                 // 3796
            method = "throw";                                                                                      // 3797
            arg = record.arg;                                                                                      // 3798
            continue;                                                                                              // 3799
          }                                                                                                        // 3800
                                                                                                                   // 3801
          // Delegate generator ran and handled its own exceptions so                                              // 3802
          // regardless of what the method was, we continue as if it is                                            // 3803
          // "next" with an undefined arg.                                                                         // 3804
          method = "next";                                                                                         // 3805
          arg = undefined;                                                                                         // 3806
                                                                                                                   // 3807
          var info = record.arg;                                                                                   // 3808
          if (info.done) {                                                                                         // 3809
            context[delegate.resultName] = info.value;                                                             // 3810
            context.next = delegate.nextLoc;                                                                       // 3811
          } else {                                                                                                 // 3812
            state = GenStateSuspendedYield;                                                                        // 3813
            return info;                                                                                           // 3814
          }                                                                                                        // 3815
                                                                                                                   // 3816
          context.delegate = null;                                                                                 // 3817
        }                                                                                                          // 3818
                                                                                                                   // 3819
        if (method === "next") {                                                                                   // 3820
          if (state === GenStateSuspendedYield) {                                                                  // 3821
            context.sent = arg;                                                                                    // 3822
          } else {                                                                                                 // 3823
            context.sent = undefined;                                                                              // 3824
          }                                                                                                        // 3825
                                                                                                                   // 3826
        } else if (method === "throw") {                                                                           // 3827
          if (state === GenStateSuspendedStart) {                                                                  // 3828
            state = GenStateCompleted;                                                                             // 3829
            throw arg;                                                                                             // 3830
          }                                                                                                        // 3831
                                                                                                                   // 3832
          if (context.dispatchException(arg)) {                                                                    // 3833
            // If the dispatched exception was caught by a catch block,                                            // 3834
            // then let that catch block handle the exception normally.                                            // 3835
            method = "next";                                                                                       // 3836
            arg = undefined;                                                                                       // 3837
          }                                                                                                        // 3838
                                                                                                                   // 3839
        } else if (method === "return") {                                                                          // 3840
          context.abrupt("return", arg);                                                                           // 3841
        }                                                                                                          // 3842
                                                                                                                   // 3843
        state = GenStateExecuting;                                                                                 // 3844
                                                                                                                   // 3845
        var record = tryCatch(innerFn, self, context);                                                             // 3846
        if (record.type === "normal") {                                                                            // 3847
          // If an exception is thrown from innerFn, we leave state ===                                            // 3848
          // GenStateExecuting and loop back for another invocation.                                               // 3849
          state = context.done                                                                                     // 3850
            ? GenStateCompleted                                                                                    // 3851
            : GenStateSuspendedYield;                                                                              // 3852
                                                                                                                   // 3853
          var info = {                                                                                             // 3854
            value: record.arg,                                                                                     // 3855
            done: context.done                                                                                     // 3856
          };                                                                                                       // 3857
                                                                                                                   // 3858
          if (record.arg === ContinueSentinel) {                                                                   // 3859
            if (context.delegate && method === "next") {                                                           // 3860
              // Deliberately forget the last sent value so that we don't                                          // 3861
              // accidentally pass it on to the delegate.                                                          // 3862
              arg = undefined;                                                                                     // 3863
            }                                                                                                      // 3864
          } else {                                                                                                 // 3865
            return info;                                                                                           // 3866
                                                                                                                   ...
                                                                                                                   // 3868
        } else if (record.type === "throw") {                                                                      // 3869
          state = GenStateCompleted;                                                                               // 3870
          // Dispatch the exception by looping back around to the                                                  // 3871
          // context.dispatchException(arg) call above.                                                            // 3872
          method = "throw";                                                                                        // 3873
          arg = record.arg;                                                                                        // 3874
        }                                                                                                          // 3875
      }                                                                                                            // 3876
                                                                                                                   ...
                                                                                                                   // 3879
  // Define Generator.prototype.{next,throw,return} in terms of the                                                // 3880
  // unified ._invoke helper method.                                                                               // 3881
  defineIteratorMethods(Gp);                                                                                       // 3882
                                                                                                                   // 3883
  Gp[iteratorSymbol] = function() {                                                                                // 3884
    return this;                                                                                                   // 3885
  };                                                                                                               // 3886
                                                                                                                   // 3887
  Gp.toString = function() {                                                                                       // 3888
    return "[object Generator]";                                                                                   // 3889
  };                                                                                                               // 3890
                                                                                                                   // 3891
  function pushTryEntry(locs) {                                                                                    // 3892
    var entry = { tryLoc: locs[0] };                                                                               // 3893
                                                                                                                   // 3894
    if (1 in locs) {                                                                                               // 3895
      entry.catchLoc = locs[1];                                                                                    // 3896
    }                                                                                                              // 3897
                                                                                                                   // 3898
    if (2 in locs) {                                                                                               // 3899
      entry.finallyLoc = locs[2];                                                                                  // 3900
      entry.afterLoc = locs[3];                                                                                    // 3901
    }                                                                                                              // 3902
                                                                                                                   // 3903
    this.tryEntries.push(entry);                                                                                   // 3904
  }                                                                                                                // 3905
                                                                                                                   // 3906
  function resetTryEntry(entry) {                                                                                  // 3907
    var record = entry.completion || {};                                                                           // 3908
    record.type = "normal";                                                                                        // 3909
    delete record.arg;                                                                                             // 3910
    entry.completion = record;                                                                                     // 3911
  }                                                                                                                // 3912
                                                                                                                   // 3913
  function Context(tryLocsList) {                                                                                  // 3914
    // The root entry object (effectively a try statement without a catch                                          // 3915
    // or a finally block) gives us a place to store values thrown from                                            // 3916
    // locations where there is no enclosing try statement.                                                        // 3917
    this.tryEntries = [{ tryLoc: "root" }];                                                                        // 3918
    tryLocsList.forEach(pushTryEntry, this);                                                                       // 3919
    this.reset(true);                                                                                              // 3920
  }                                                                                                                // 3921
                                                                                                                   // 3922
  runtime.keys = function(object) {                                                                                // 3923
    var keys = [];                                                                                                 // 3924
    for (var key in object) {                                                                                      // 3925
      keys.push(key);                                                                                              // 3926
    }                                                                                                              // 3927
    keys.reverse();                                                                                                // 3928
                                                                                                                   // 3929
    // Rather than returning an object with a next method, we keep                                                 // 3930
    // things simple and return the next function itself.                                                          // 3931
    return function next() {                                                                                       // 3932
      while (keys.length) {                                                                                        // 3933
        var key = keys.pop();                                                                                      // 3934
        if (key in object) {                                                                                       // 3935
          next.value = key;                                                                                        // 3936
          next.done = false;                                                                                       // 3937
          return next;                                                                                             // 3938
        }                                                                                                          // 3939
      }                                                                                                            // 3940
                                                                                                                   // 3941
      // To avoid creating an additional object, we just hang the .value                                           // 3942
      // and .done properties off the next function object itself. This                                            // 3943
      // also ensures that the minifier will not anonymize the function.                                           // 3944
      next.done = true;                                                                                            // 3945
      return next;                                                                                                 // 3946
    };                                                                                                             // 3947
  };                                                                                                               // 3948
                                                                                                                   // 3949
  function values(iterable) {                                                                                      // 3950
    if (iterable) {                                                                                                // 3951
      var iteratorMethod = iterable[iteratorSymbol];                                                               // 3952
      if (iteratorMethod) {                                                                                        // 3953
        return iteratorMethod.call(iterable);                                                                      // 3954
      }                                                                                                            // 3955
                                                                                                                   // 3956
      if (typeof iterable.next === "function") {                                                                   // 3957
        return iterable;                                                                                           // 3958
      }                                                                                                            // 3959
                                                                                                                   // 3960
      if (!isNaN(iterable.length)) {                                                                               // 3961
        var i = -1, next = function next() {                                                                       // 3962
          while (++i < iterable.length) {                                                                          // 3963
            if (hasOwn.call(iterable, i)) {                                                                        // 3964
              next.value = iterable[i];                                                                            // 3965
              next.done = false;                                                                                   // 3966
              return next;                                                                                         // 3967
            }                                                                                                      // 3968
          }                                                                                                        // 3969
                                                                                                                   // 3970
          next.value = undefined;                                                                                  // 3971
          next.done = true;                                                                                        // 3972
                                                                                                                   // 3973
          return next;                                                                                             // 3974
        };                                                                                                         // 3975
                                                                                                                   // 3976
        return next.next = next;                                                                                   // 3977
      }                                                                                                            // 3978
    }                                                                                                              // 3979
                                                                                                                   // 3980
    // Return an iterator with no values.                                                                          // 3981
    return { next: doneResult };                                                                                   // 3982
  }                                                                                                                // 3983
  runtime.values = values;                                                                                         // 3984
                                                                                                                   // 3985
  function doneResult() {                                                                                          // 3986
    return { value: undefined, done: true };                                                                       // 3987
  }                                                                                                                // 3988
                                                                                                                   // 3989
  Context.prototype = {                                                                                            // 3990
    constructor: Context,                                                                                          // 3991
                                                                                                                   // 3992
    reset: function(skipTempReset) {                                                                               // 3993
      this.prev = 0;                                                                                               // 3994
      this.next = 0;                                                                                               // 3995
      this.sent = undefined;                                                                                       // 3996
      this.done = false;                                                                                           // 3997
      this.delegate = null;                                                                                        // 3998
                                                                                                                   // 3999
      this.tryEntries.forEach(resetTryEntry);                                                                      // 4000
                                                                                                                   // 4001
      if (!skipTempReset) {                                                                                        // 4002
        for (var name in this) {                                                                                   // 4003
          // Not sure about the optimal order of these conditions:                                                 // 4004
          if (name.charAt(0) === "t" &&                                                                            // 4005
              hasOwn.call(this, name) &&                                                                           // 4006
              !isNaN(+name.slice(1))) {                                                                            // 4007
            this[name] = undefined;                                                                                // 4008
          }                                                                                                        // 4009
        }                                                                                                          // 4010
      }                                                                                                            // 4011
    },                                                                                                             // 4012
    ...
    stop: function() {                                                                                             // 4014
      this.done = true;                                                                                            // 4015
                                                                                                                   // 4016
      var rootEntry = this.tryEntries[0];                                                                          // 4017
      var rootRecord = rootEntry.completion;                                                                       // 4018
      if (rootRecord.type === "throw") {                                                                           // 4019
        throw rootRecord.arg;                                                                                      // 4020
      }                                                                                                            // 4021
                                                                                                                   // 4022
      return this.rval;                                                                                            // 4023
    },                                                                                                             // 4024
                                                                                                                   // 4025
    dispatchException: function(exception) {                                                                       // 4026
      if (this.done) {                                                                                             // 4027
        throw exception;                                                                                           // 4028
      }                                                                                                            // 4029
                                                                                                                   // 4030
      var context = this;                                                                                          // 4031
      function handle(loc, caught) {                                                                               // 4032
        record.type = "throw";                                                                                     // 4033
        record.arg = exception;                                                                                    // 4034
        context.next = loc;                                                                                        // 4035
        return !!caught;                                                                                           // 4036
      }                                                                                                            // 4037
                                                                                                                   // 4038
      for (var i = this.tryEntries.length - 1; i >= 0; --i) {                                                      // 4039
        var entry = this.tryEntries[i];                                                                            // 4040
        var record = entry.completion;                                                                             // 4041
                                                                                                                   // 4042
        if (entry.tryLoc === "root") {                                                                             // 4043
          // Exception thrown outside of any try block that could handle                                           // 4044
          // it, so set the completion value of the entire function to                                             // 4045
          // throw the exception.                                                                                  // 4046
          return handle("end");                                                                                    // 4047
        }                                                                                                          // 4048
                                                                                                                   // 4049
        if (entry.tryLoc <= this.prev) {                                                                           // 4050
          var hasCatch = hasOwn.call(entry, "catchLoc");                                                           // 4051
          var hasFinally = hasOwn.call(entry, "finallyLoc");                                                       // 4052
                                                                                                                   // 4053
          if (hasCatch && hasFinally) {                                                                            // 4054
            if (this.prev < entry.catchLoc) {                                                                      // 4055
              return handle(entry.catchLoc, true);                                                                 // 4056
            } else if (this.prev < entry.finallyLoc) {                                                             // 4057
              return handle(entry.finallyLoc);                                                                     // 4058
            }                                                                                                      // 4059
                                                                                                                   // 4060
          } else if (hasCatch) {                                                                                   // 4061
            if (this.prev < entry.catchLoc) {                                                                      // 4062
              return handle(entry.catchLoc, true);                                                                 // 4063
            }                                                                                                      // 4064
                                                                                                                   // 4065
          } else if (hasFinally) {                                                                                 // 4066
            if (this.prev < entry.finallyLoc) {                                                                    // 4067
              return handle(entry.finallyLoc);                                                                     // 4068
            }                                                                                                      // 4069
                                                                                                                   // 4070
          } else {                                                                                                 // 4071
            throw new Error("try statement without catch or finally");                                             // 4072
          }                                                                                                        // 4073
        }                                                                                                          // 4074
      }                                                                                                            // 4075
    },                                                                                                             // 4076
                                                                                                                   // 4077
    abrupt: function(type, arg) {                                                                                  // 4078
      for (var i = this.tryEntries.length - 1; i >= 0; --i) {                                                      // 4079
        var entry = this.tryEntries[i];                                                                            // 4080
        if (entry.tryLoc <= this.prev &&                                                                           // 4081
            hasOwn.call(entry, "finallyLoc") &&                                                                    // 4082
            this.prev < entry.finallyLoc) {                                                                        // 4083
          var finallyEntry = entry;                                                                                // 4084
          break;                                                                                                   // 4085
        }                                                                                                          // 4086
      }                                                                                                            // 4087
                                                                                                                   // 4088
      if (finallyEntry &&                                                                                          // 4089
          (type === "break" ||                                                                                     // 4090
           type === "continue") &&                                                                                 // 4091
          finallyEntry.tryLoc <= arg &&                                                                            // 4092
          arg <= finallyEntry.finallyLoc) {                                                                        // 4093
        // Ignore the finally entry if control is not jumping to a                                                 // 4094
        // location outside the try/catch block.                                                                   // 4095
        finallyEntry = null;                                                                                       // 4096
      }                                                                                                            // 4097
                                                                                                                   // 4098
      var record = finallyEntry ? finallyEntry.completion : {};                                                    // 4099
      record.type = type;                                                                                          // 4100
      record.arg = arg;                                                                                            // 4101
                                                                                                                   // 4102
      if (finallyEntry) {                                                                                          // 4103
        this.next = finallyEntry.finallyLoc;                                                                       // 4104
      } else {                                                                                                     // 4105
        this.complete(record);                                                                                     // 4106
      }                                                                                                            // 4107
                                                                                                                   // 4108
      return ContinueSentinel;                                                                                     // 4109
    },                                                                                                             // 4110
                                                                                                                   // 4111
    complete: function(record, afterLoc) {                                                                         // 4112
      if (record.type === "throw") {                                                                               // 4113
        throw record.arg;                                                                                          // 4114
      }                                                                                                            // 4115
                                                                                                                   // 4116
      if (record.type === "break" ||                                                                               // 4117
          record.type === "continue") {                                                                            // 4118
        this.next = record.arg;                                                                                    // 4119
      } else if (record.type === "return") {                                                                       // 4120
        this.rval = record.arg;                                                                                    // 4121
        this.next = "end";                                                                                         // 4122
      } else if (record.type === "normal" && afterLoc) {                                                           // 4123
        this.next = afterLoc;                                                                                      // 4124
      }                                                                                                            // 4125
    },                                                                                                             // 4126
                                                                                                                   // 4127
    finish: function(finallyLoc) {                                                                                 // 4128
      for (var i = this.tryEntries.length - 1; i >= 0; --i) {                                                      // 4129
        var entry = this.tryEntries[i];                                                                            // 4130
        if (entry.finallyLoc === finallyLoc) {                                                                     // 4131
          this.complete(entry.completion, entry.afterLoc);                                                         // 4132
          resetTryEntry(entry);                                                                                    // 4133
          return ContinueSentinel;                                                                                 // 4134
        }                                                                                                          // 4135
      }                                                                                                            // 4136
    },                                                                                                             // 4137
                                                                                                                   // 4138
    "catch": function(tryLoc) {                                                                                    // 4139
      for (var i = this.tryEntries.length - 1; i >= 0; --i) {                                                      // 4140
        var entry = this.tryEntries[i];                                                                            // 4141
        if (entry.tryLoc === tryLoc) {                                                                             // 4142
          var record = entry.completion;                                                                           // 4143
          if (record.type === "throw") {                                                                           // 4144
            var thrown = record.arg;                                                                               // 4145
            resetTryEntry(entry);                                                                                  // 4146
          }                                                                                                        // 4147
          return thrown;                                                                                           // 4148
        }                                                                                                          // 4149
      }                                                                                                            // 4150
                                                                                                                   // 4151
      // The context.catch method must only be called with a location                                              // 4152
      // argument that corresponds to a known catch block.                                                         // 4153
      throw new Error("illegal catch attempt");                                                                    // 4154
    },                                                                                                             // 4155
                                                                                                                   // 4156
    delegateYield: function(iterable, resultName, nextLoc) {                                                       // 4157
      this.delegate = {                                                                                            // 4158
        iterator: values(iterable),                                                                                // 4159
        resultName: resultName,                                                                                    // 4160
        nextLoc: nextLoc                                                                                           // 4161
      };                                                                                                           // 4162
                                                                                                                   // 4163
      return ContinueSentinel;                                                                                     // 4164
    }                                                                                                              // 4165
  };                                                                                                               // 4166
})(                                                                                                                // 4167
  // Among the various tricks for obtaining a reference to the global                                              // 4168
  // object, this seems to be the most reliable technique that does not                                            // 4169
  // use indirect eval (which violates Content Security Policy).                                                   // 4170
  typeof global === "object" ? global :                                                                            // 4171
  typeof window === "object" ? window :                                                                            // 4172
  typeof self === "object" ? self : this                                                                           // 4173
);                                                                                                                 // 4174
                                                                                                                   // 4175
}).call(this,typeof global !== "undefined" ? global : typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
},{}]},{},[1]);                                                                                                    // 4177
